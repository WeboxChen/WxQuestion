...
namespace Wei.Web.Controllers
{
    public class CommonController : BaseController
    {
        ...
        [JsonCallback]
        [HttpPost]
        public object GetView(RequestPaging paras)
        {
            string viewname = paras.ExtraView;
            string filter = GetFilter(paras.Filter);
            string sort = string.IsNullOrEmpty(paras.Sort) ? string.Format(" id {0} ", paras.Direction) : GetSort(paras.Sort);
            int count, fcount;
        ...
        {
            int count, fcount;
            string filter = GetFilter(request.Filter);
            string sort = string.IsNullOrEmpty(request.Sort) ? string.Format(" id {0} ", request.Direction) : GetSort(request.Sort);
